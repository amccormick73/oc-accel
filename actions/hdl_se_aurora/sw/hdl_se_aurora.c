...
static int run_se_aurora (struct snap_card* h,
        uint32_t timeout,
        void* src_base,
        void* tgt_base,
			  uint32_t size,
        ...
        )
{
    ...
    VERBOSE0 (" ----- Check Aurora REG ----- \n");
    reg_data = action_read(h, REG_AU0_STATUS);
    printf("Aurora Status: %8.8x\n", reg_data);

   
    VERBOSE0 (" ----- Reset Aurora ----- \n");
    action_write(h, REG_AU0_CTRL, 5);
    usleep(1);
    action_write(h, REG_AU0_CTRL, 1);
    usleep(10);
    action_write(h, REG_AU0_CTRL, 0);
        ...
        snap_action_flag_t flags, uint32_t timeout)
{
    ...
    act = snap_attach_action (handle, ACTION_TYPE_HDL_SE_AURORA,
            flags, timeout);

    if (NULL == act) {
        VERBOSE0 ("Error: Can not attach Action: %x\n", ACTION_TYPE_HDL_SE_AURORA);
