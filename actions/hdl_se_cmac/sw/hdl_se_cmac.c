...
static int run_se_cmac (struct snap_card* h,
        uint32_t timeout,
        void* src_base,
        void* tgt_base,
			  uint32_t size,
        ...
        )
{
    ...
    VERBOSE0 (" ----- Check CMAC REG ----- \n");
    reg_data = action_read(h, REG_CMAC0_STATUS);
    printf("Aurora Status: %8.8x\n", reg_data);

   
    VERBOSE0 (" ----- Reset CMAC ----- \n");
    action_write(h, REG_CMAC0_CTRL, CMAC_SYS_RESET);
    usleep(10);
    action_write(h, REG_CMAC0_CTRL, 0);
    ...
    action_write(h, REG_CMAC0_CTRL,CMAC_TX_SEND_RFI);

    VERBOSE0 (" ----- RX Enable ----- \n");


    usleep(10);
    action_write(h, REG_CMAC0_CTRL+8, 0x4);

    VERBOSE0 (" ----- Check CMAC REG ----- \n");



    do { // RETRY IF PCSL DEMUX IS NOT ALL ONES

    cnt = 0;
    do { 
      usleep(100);
        reg_data = action_read(h, REG_CMAC0_STATUS);
    printf("CMAC Status: %8.8x\n", reg_data);
            
    if ((reg_data &0x3F) == 0xF )
      break;
        cnt ++;
    } while (cnt < timeout * 10); //Use timeout to emulate max allowed MMIO read times

    VERBOSE0 (" ----- Check CMAC RX_BLOCK_LOCK REG ----- \n");
    ...
    do { 
      usleep(100);
        reg_data = action_read(h, REG_CMAC0_STATUS+68);
    printf("CMAC BLOCK LOCK Status: %8.8x\n", reg_data);
            
    if ((reg_data & 0x07FFFF80) == 0x07FFFF80 )
      break;
        cnt ++;
    } while (cnt < timeout * 10); //Use timeout to emulate max allowed MMIO read times
 

VERBOSE0 (" ----- Check CMAC RX_SYNC REG ----- \n");
    
    cnt = 0;
    do { 
      usleep(100);
        reg_data = action_read(h, REG_CMAC0_STATUS+32);
    printf("CMAC RX_SYNC Status: %8.8x\n", reg_data);
            
    if ((reg_data & 0x000FFFFF) == 0x000FFFFF )
      break;
        cnt ++;
    } while (cnt < timeout * 200); //Use timeout to emulate max allowed MMIO read times


    VERBOSE0 (" ----- Check CMAC RX_PCSL_DEMUX REG ----- \n");
   
    reg_data = action_read(h, REG_CMAC0_STATUS+20); 
    printf("CMAC RX_PCSL_DEMUX Status (bits0-5): %8.8x\n", reg_data);
    reg_data2 = action_read(h, REG_CMAC0_STATUS+24);
    printf("CMAC RX_PCSL_DEMUX Status (bits6-19): %8.8x\n", reg_data2);

    if (((reg_data &0xFC000000)==0xFC000000) && ((reg_data2&0x3FFF)==0x3FFF))
      break;
   
    VERBOSE0 (" ----- CMAC PCS Lanes incorrectly demuxed: forcing resync----- \n");
    usleep(10);
    action_write(h, REG_CMAC0_CTRL+8, 0x2);
    usleep(10);
    action_write(h, REG_CMAC0_CTRL+8, 0x4);
...
VERBOSE0 (" ----- Check CMAC REG for RX_STATUS bit----- \n");
 ...
 cnt = 0;
    ...
    VERBOSE0 (" ----- TX Enable and RX Pause Enable ----- \n");
    action_write(h, REG_CMAC0_CTRL+4, 0xF00FF0);
    action_write(h, REG_CMAC0_CTRL+8, 0xFFC);


    action_write(h, REG_CMAC0_CTRL, CMAC_TX_ENABLE);
    
   
    VERBOSE0 (" ----- Check CMAC REG for RX_STATUS bit set----- \n");
...
VERBOSE0 (" ----- Check CMAC REG for RX_STATUS bit clear----- \n");
 ...
 cnt = 0;
    ...
    if ((reg_data &0x80) == 0x0 )
      break;
        cnt ++;
    } while (cnt < timeout * 200); //Use timeout to emulate max allowed MMIO read times

VERBOSE0 (" ----- Set up a TX Packet ----- \n");


    action_write(h, REG_TX0_CMD0, CMD_EOF | CMD_INCR | size);
    action_write(h, REG_TX0_CMD1, (uint32_t) (((uint64_t) src_base) & 0xffffffff));
    action_write(h, REG_TX0_CMD2, (uint32_t) ((((uint64_t) src_base) >> 32) & 0xffffffff));
    

    VERBOSE0 (" ----- Set up an RX Packet ----- \n");
    action_write(h, REG_RX0_CMD0, CMD_EOF | CMD_INCR | size);
    action_write(h, REG_RX0_CMD1, (uint32_t) (((uint64_t) tgt_base) & 0xffffffff));
    action_write(h, REG_RX0_CMD2, (uint32_t) ((((uint64_t) tgt_base) >> 32) & 0xffffffff));



    VERBOSE0 (" ----- Tell AFU to kick off TX and RX AXI transactions ----- \n");
    action_write(h, REG_TX0_CONTROL, 0x00000001);
    action_write(h, REG_RX0_CONTROL, 0x00000001);
        ...
        snap_action_flag_t flags, uint32_t timeout)
{
    ...
    act = snap_attach_action (handle, ACTION_TYPE_HDL_SE_CMAC,
            flags, timeout);

    if (NULL == act) {
        VERBOSE0 ("Error: Can not attach Action: %x\n", ACTION_TYPE_HDL_SE_CMAC);
...
int main (int argc, char* argv[])
{
    ...
    rnum = 1;
    wnum = 1;
    rpattern = 0x00000707; //ID < 4, Len=1F, Size=7 
    wpattern = 0x00000707; //ID < 4, Len=1F, Size=7
    ...
    if(wrap_pattern == 0){
        ...
        mem_init (exp_buff, init_rdata, wtotal_bytes);
        ...
        mem_init (exp_buff, init_rdata, size_128MB);
