...
set_property -dict [list CONFIG.c_m_axi_mm2s_data_width {1024} CONFIG.c_m_axis_mm2s_tdata_width {512} CONFIG.c_mm2s_burst_size {2} CONFIG.c_m_axi_s2mm_data_width {1024} CONFIG.c_s_axis_s2mm_tdata_width {512} CONFIG.c_s2mm_burst_size {2} CONFIG.c_s2mm_support_indet_btt {true} CONFIG.c_s2mm_include_sf {false} CONFIG.c_enable_cache_user {false} CONFIG.c_enable_mm2s_adv_sig {0} CONFIG.c_enable_s2mm_adv_sig {0} CONFIG.c_addr_width {64}] [get_ips axi_datamover_0]
...
set_property -dict [list CONFIG.TDATA_NUM_BYTES {64} CONFIG.FIFO_DEPTH {8192} CONFIG.HAS_TLAST {1} CONFIG.HAS_TKEEP {1} CONFIG.HAS_RD_DATA_COUNT {1} CONFIG.IS_ACLK_ASYNC {1} CONFIG.FIFO_MODE {2}] [get_ips axis_data_fifo_data]
...
create_ip -name cmac_usplus -vendor xilinx.com -library ip -version 2.6 -module_name cmac_usplus_0 -dir $src_dir >> $log_file
set_property -dict  [list CONFIG.CMAC_CAUI4_MODE {1} CONFIG.NUM_LANES {4} CONFIG.GT_REF_CLK_FREQ {161.1328125} CONFIG.USER_INTERFACE {AXIS} CONFIG.GT_DRP_CLK {200} CONFIG.CMAC_CORE_SELECT {CMACE4_X0Y2} CONFIG.GT_GROUP_SELECT {X0Y16~X0Y19} CONFIG.LANE1_GT_LOC {X0Y16} CONFIG.LANE2_GT_LOC {X0Y17} CONFIG.LANE3_GT_LOC {X0Y18} CONFIG.LANE4_GT_LOC {X0Y19} CONFIG.LANE5_GT_LOC {NA} CONFIG.LANE6_GT_LOC {NA} CONFIG.LANE7_GT_LOC {NA} CONFIG.LANE8_GT_LOC {NA} CONFIG.LANE9_GT_LOC {NA} CONFIG.LANE10_GT_LOC {NA}] [get_ips cmac_usplus_0]
generate_target {instantiation_template} [get_files $src_dir/cmac_usplus_0/cma_usplus_0.xci]  >> $log_file
generate_target all [get_files $src_dir/cmac_usplus_0/cmac_usplus_0.xci]  >> $log_file
