...
static int run_se_dmdemo (struct snap_card* h,
        uint32_t timeout,
        void* src_base,
        void* tgt_base,
			  uint32_t size,
        ...
        )
{
    ...
    action_write(h, REG_TX0_CMD0, CMD_INCR | size);
    ...
    action_write(h, REG_RX0_CMD0, CMD_INCR | size);
        ...
        snap_action_flag_t flags, uint32_t timeout)
{
    ...
    act = snap_attach_action (handle, ACTION_TYPE_HDL_SE_DMDEMO,
            flags, timeout);

    if (NULL == act) {
        VERBOSE0 ("Error: Can not attach Action: %x\n", ACTION_TYPE_HDL_SE_DMDEMO);
